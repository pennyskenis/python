...
#adds value ordered menu and gourmet ordered menu
#prints the full ordered menu back to the user

    full_ordered_menu = value_ordered_menu + gourmet_ordered_menu
    print("The pizzas you have chosen to order are as follows:")
    print(*full_ordered_menu, sep='\n')

#extra toppings prompt
#while loops make it so the user must give a valid input
#blank input is deemed invalid > user feedback
    while True:
        try:
            finalise_order = int(input("\nWould you like to add extra toppings to your pizzas for an additional 50c (per pizza)?\n* press 1 to add extra toppings \n* press 0 to skip\n"))
        except ValueError:
            print("Sorry but I don't understand.")

#if user does not want extra toppings > no toppings are added > loop breaks
#final order statement is printed
        #price
        #user name
        #delivery address
        #phone number
        ...
        if finalise_order == 0:
            print("\n~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~")
            print("\n\nHere is the list of pizzas you have chose:")
            print(*full_ordered_menu, sep='\n')
            if method == 1:
                print("\nKa Pai! {0}. Your order comes to a total of ${1}, \n * the delivery address is {2} \n * and if needed - we will contact you with the phone number {3}".format(user_name, total_price, delivery_address, phone_number))
                print("~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~")
            if method == 2:
                print("\nKa Pai! {0}. Your order comes to a total of ${1}, \n * and if needed - we will contact you with the phone number {3}".format(user_name, total_price, delivery_address, phone_number))
                print("~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~")
...
#if user wants extra toppings > list of ordered pizzas is displayed back to the user with extra topping information
        # user feedback
#final order statement is printed
        #price
        #user name
        ...
        if finalise_order == 1:
            print("\n~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~")
            print("Okay! Extra topppings have been added to your order!")
            print("\n\nHere is the list of pizzas you have chose:")
            total_pizza_sum = value_pizza_amount + gourmet_pizza_amount
            total_sum_xtratoppings = total_price + total_pizza_sum * toppings_price
            print(*full_ordered_menu, "+ $0.50 to ALL pizza prices", sep='\n')
            if method == 1:
                print("\nKa Pai! {0}. Your order comes to a total of ${1}, \n * the delivery address is {2} \n * and if needed - we will contact you with the phone number {3}".format(user_name, total_sum_xtratoppings, delivery_address, phone_number))
                print("~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~")
            if method == 2:
                print("\nKa Pai! {0}. Your order comes to a total of ${1}, \n * and if needed - we will contact you with the phone number {3}".format(user_name, total_sum_xtratoppings, delivery_address, phone_number))
                print("~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~")
...
#while loops make it so the user must give a valid input
#blank input is deemed invalid > user feedback
#if finish = 0 > order is cancelled > program exits
#if finish = 1 > order is complete > loop breaks

#user is able to complete other pizza order form
        #all code is in a loop
        #loop only broken if user exits program
        
    while True:
        try:
            finish = int(input("\nAlmost there! Please confirm your order\n* press 1 to confirm your order\n* press 0 cancel to your order\n"))
        except ValueError:
            print("Sorry but I don't understand.")
        ...
        if finish == 1:
            print("Thank you for buying from the Henderson Pizza Palace - Enjoy!\n\n\n ***End of Order***\n\n")
            break
